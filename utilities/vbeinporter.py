"""
VB-BE (Vital Bracelet BE) Digimon Data Importer
Adapted to handle the new VB-BE format with:
- PP (Performance Points) instead of Trophies
- BP (Battle Power) instead of DP
- Activity Type instead of specific sleep/wake times
...
# Default values by stage (adapted for VB-BE format with higher stats)
...
def parse_evolution_conditions(conditions_text):
    ...
    # PP (Performance Points) - replaces Trophies
    if match := re.search(r"(\d+)\+?\s*PP", conditions_text, re.I):
        conditions["trophies"] = [int(match.group(1)), 999999]
    
    # Trophies (for backward compatibility)
    ...
    # Stage clearing requirements
    if match := re.search(r"Stage\s+(\d+)\s+cleared", conditions_text, re.I):
        conditions["stage_cleared"] = int(match.group(1))
    
    # Handle range-based conditions (e.g., "0-4 PP", "0-499 Vital Values")
    # PP ranges
    if match := re.search(r"(\d+)-(\d+)\s*PP", conditions_text, re.I):
        min_pp, max_pp = int(match.group(1)), int(match.group(2))
        # For ranges like 0-4, we interpret as "less than or equal to max"
        if min_pp == 0:
            conditions["trophies"] = [0, max_pp]
        else:
            conditions["trophies"] = [min_pp, max_pp]
    
    # Vital Values ranges
    if match := re.search(r"(\d+)-(\d+)\s*Vital Values", conditions_text, re.I):
        min_vv, max_vv = int(match.group(1)), int(match.group(2))
        if min_vv == 0:
            conditions["vital_values"] = [0, max_vv]
        else:
            conditions["vital_values"] = [min_vv, max_vv]
    
    # Battle ranges
    if match := re.search(r"(\d+)-(\d+)\s*Battles", conditions_text, re.I):
        min_battles, max_battles = int(match.group(1)), int(match.group(2))
        if min_battles == 0:
            conditions["battles"] = [0, max_battles]
        else:
            conditions["battles"] = [min_battles, max_battles]
    
    # Handle "Stage X not cleared" conditions
    if match := re.search(r"Stage\s+(\d+)\s+not cleared", conditions_text, re.I):
        conditions["stage_not_cleared"] = int(match.group(1))
...
def get_digimon_details(code, device="14"):
...
def extract_digimon_info(details_soup):
    ...
    for bj_div in bj_divs:
        ...
        # BP (Battle Power) - replaces DP
        if match := re.search(r"BP:\s*(\d+)", text):
            info["power"] = int(match.group(1))
        
        # DP (Power) and Star (for backward compatibility)
        ...
        # Sleep/Wake times (old format)
        if match := re.search(r"Awake from:\s*(\d{2}:\d{2})-(\d{2}:\d{2})", text):
            info["wakes"] = match.group(1)
            info["sleeps"] = match.group(2)
        
        # Activity Type (new format)
        if match := re.search(r"Activity Type:\s*(\w+)", text):
            activity_type = match.group(1).lower()
            # Map activity types to sleep/wake times
            if activity_type == "normal":
                info["wakes"] = "08:00"
                info["sleeps"] = "21:00"
            elif activity_type == "nocturnal":
                info["wakes"] = "20:00"
                info["sleeps"] = "08:00"
    ...
    if evo_section:
        ...
        for evo in evolutions:
            ...
            if name_elem:
                ...
                conditions = {}
                if next_p:
                    conditions_text = next_p.get_text()
                    conditions = parse_evolution_conditions(conditions_text)
                    
                    # Handle list-based conditions ("Choose one:")
                    if "Choose one:" in conditions_text:
                        deets_list = next_p.find_next_sibling("ul", class_="deetsList")
                        if deets_list:
                            list_items = deets_list.find_all("li")
                            for item in list_items:
                                item_text = item.get_text()
                                item_conditions = parse_evolution_conditions(item_text)
                                # For "choose one" conditions, we'll take the first valid condition
                                # or create alternative evolution paths
                                if not conditions and item_conditions:
...
def import_vb_data(html_file_path=None):
    ...
    with open("vbbe_digimons.json", "w", encoding="utf-8") as f:
        json.dump(output_data, f, indent=2, ensure_ascii=False)
    
    print(f"Imported {len(all_digimons)} digimons to vbbe_digimons.json")
    return output_data
