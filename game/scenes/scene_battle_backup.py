...
class SceneBattle:
            ...
            if cache_key != self._cache_key or self._cache_surface is None:
                ...
                if self.phase in ["menu", "module", "battle_select"]:
                    ...
                    if len(self.menu.options) > 2:
                        self.menu.draw(cache_surface, constants.SCREEN_WIDTH // 2 - self.menu.get_width() // 2, int(100 * constants.UI_SCALE))
                    else:
                        self.menu.draw(cache_surface, constants.SCREEN_WIDTH // 2 - self.menu.get_width() // 2, int(120 * constants.UI_SCALE))
        ...
        for i in range(2):
            ...
            if i == 1:
                if self.armor_digimental_items:
                    ...
                    item_data = self.armor_digimental_items[idx]
                    item = item_data["item"]
                    module = item_data["module"]
                    
                    # Draw item sprite
                    try:
                        item_sprite_path = f"{module.folder_path}/{item.sprite}"
                        item_sprite = sprite_load_percent(item_sprite_path, percent=30, keep_proportion=True, base_on="height")
                        item_x = x + (self.jogress_slot_on.get_width() - item_sprite.get_width()) // 2
                        item_y = y + (self.jogress_slot_on.get_height() - item_sprite.get_height()) // 2
                        blit_with_shadow(surface, item_sprite, (item_x, item_y))
                    except Exception as e:
                        runtime_globals.game_console.log(f"[SceneBattle] Error loading item sprite {item.sprite}: {e}")
                        # Draw a placeholder or item name
                        font = get_font(int(12 * constants.UI_SCALE))
                        text = font.render(item.name[:8], True, (255, 255, 255))
                        text_x = x + (self.jogress_slot_on.get_width() - text.get_width()) // 2
                        text_y = y + (self.jogress_slot_on.get_height() - text.get_height()) // 2
                        surface.blit(text, (text_x, text_y))
                        
                    # Draw item name below
                    font = get_font(int(10 * constants.UI_SCALE))
                    name_text = font.render(item.name, True, (255, 255, 255))
                    name_x = x + (self.jogress_slot_on.get_width() - name_text.get_width()) // 2
                    name_y = y + self.jogress_slot_on.get_height() + int(5 * constants.UI_SCALE)
                    surface.blit(name_text, (name_x, name_y))
                    
                    # Draw quantity
                    quantity = get_inventory_value(item.name)
                    qty_text = font.render(f"x{quantity}", True, (200, 200, 200))
                    qty_x = x + self.jogress_slot_on.get_width() - qty_text.get_width() - int(5 * constants.UI_SCALE)
                    qty_y = y + int(5 * constants.UI_SCALE)
                    surface.blit(qty_text, (qty_x, qty_y))
                else:
                    # No digimental items
                    font = get_font(int(14 * constants.UI_SCALE))
                    text = font.render("No Items", True, (128, 128, 128))
                    text_x = x + (self.jogress_slot_on.get_width() - text.get_width()) // 2
                    text_y = y + (self.jogress_slot_on.get_height() - text.get_height()) // 2
                    surface.blit(text, (text_x, text_y))
        ...
        if cache_key not in self._unlocked_pairs_cache:
            ...
            for a in range(1, max_area + 1):
                if a == max_area:
                    ...
                    max_round = area_round_counts.get(a, 5)
            ...
            if self.armor_ready:
                ...
                for evo in getattr(pet, "evolve", []):
                    if "item" in evo and evo["item"] == digimental_item.name:
                        ...
                        runtime_globals.game_console.log(f"[Armor Evolution] {pet.name} evolved to {evo['name']}")
                        remove_from_inventory(digimental_item.name, 1)
                        pet.evolve_to(
                            new_name=evo['name'],
                            new_stage=evo.get('stage', pet.stage),
                            new_attribute=evo.get('attribute', pet.attribute),
                            new_level=evo.get('level', pet.level),
                            new_version=evo.get('version', pet.version)
                        )
                        break
    ...
    def perform_jogress(self):
        ...
        for evo in pet1.evolve:
            ...
            if evo.get("jogress_prefix", False):
                if pet2.name.startswith(evo["jogress"]):
                    self.execute_jogress(pet1, pet2, evo)
                    return
    ...
    def execute_jogress(self, pet1, pet2, evolution):
        runtime_globals.game_sound.play("evolution")
        runtime_globals.game_console.log(f"[Jogress] {pet1.name} + {pet2.name} = {evolution['name']}")
        
        # Remove pet2 and evolve pet1
        game_globals.pet_list.remove(pet2)
        pet1.evolve_to(
            new_name=evolution['name'],
            new_stage=evolution.get('stage', pet1.stage),
            new_attribute=evolution.get('attribute', pet1.attribute),
            new_level=evolution.get('level', pet1.level),
            new_version=evolution.get('version', pet1.version)
        )
        ...
        for module in runtime_globals.game_modules.values():
            if hasattr(module, "items"):
                for item in module.items:
                    if hasattr(item, "type") and item.type == "digimental" and get_inventory_value(item.name) > 0:
                        digimental_items.append({"item": item, "module": module})
