...
class DigidexTree(UIComponent):
    ...
    def _build_tree_surface(self):
        ...
        margin = int(50 * constants.UI_SCALE)
        ...
        for (x_idx, y_idx), name in self.tree_node_grid.items():
            ...
            for child_name in children:
                ...
                pygame.draw.line(self.tree_surface, color, (px1, py1), (px2, py2), int(3 * constants.UI_SCALE))
        ...
        for name, (px, py) in self.tree_node_pos.items():
            ...
            pygame.draw.rect(
                ...
                (screen_x - int(4 * constants.UI_SCALE), screen_y - int(4 * constants.UI_SCALE), 
                 sprite_size + int(8 * constants.UI_SCALE), sprite_size + int(8 * constants.UI_SCALE))
            )
            ...
            pygame.draw.rect(self.tree_surface, color2, sprite_rect, int(2 * constants.UI_SCALE))

            if pet and pet.known:
                label = self.font.render(pet.name, True, color)
                blit_with_shadow(self.tree_surface, label, (screen_x, screen_y + sprite_size + int(2 * constants.UI_SCALE)))
