...
class QuestPanel(UIComponent):
    ...
    def _load_icons(self):
        ...
        try:
            ...
            if os.path.exists(heart_path):
                self.heart_icon = pygame.image.load(heart_path).convert_alpha()
    ...
    def _draw_quest_content(self, surface, padding):
        ...
        # Module name or status (bottom left)
        module_font = get_font(constants.FONT_SIZE_SMALL* self.manager.ui_scale)
        if self.quest.status == QuestStatus.SUCCESS:
        ...
        # Quest progress (top right)
        progress_font = get_font(constants.FONT_SIZE_MEDIUM* self.manager.ui_scale)
        if self.quest.status == QuestStatus.FINISHED:
            progress_text = "DONE"
            progress_color = GRAY
        else:
    ...
    def _draw_reward(self, surface, padding):
        ...
        reward_font = get_font(constants.FONT_SIZE_SMALL* self.manager.ui_scale)
    ...
    def _draw_empty_content(self, surface):
        ...
        empty_font = get_font(constants.FONT_SIZE_MEDIUM)
